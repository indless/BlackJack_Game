...
class Dealer(object):
    ...
    def deal_round(self):
        ...
        for p in self.player_list:
            if p.balance() == 0:
                print('{name}, your balance is ${bank}. Would you like to buy back in for ${init}?'
                      .format(name=p.get_name(), bank=p.balance(), init=self.starting_bankroll))
                self.input = ''
                while self.input not in 'yes no 1 2'.split():
                    self.input = input('yes/no ')
                if self.input in 'yes 1'.split():
                    p.deposit(self.starting_bankroll)
                else:
                    self.player_list.remove(p)
                    # reassign player identity to new index in self.player_list
                    for rp in range(len(self.player_list)):
                        self.player_list[rp].identity = rp
                    break
            p.make_bet()

        # if there are no players with >0 bank in the player list, quit application
        ...
        for h in self.player_hands:
            if ((h.get_hand_value() > self.dealer_hand.get_hand_value() or self.dealer_hand.bust) and not h.bust) \
                    or h.black_jack():
                self.hand_wins(h)
    ...
    def get_player_move(self, hand):
        ...
        if hand.bust:
            print(hand.hand)
            print('{hv}, {name}, player #{num}, Busted!'
                  .format(hv=hand.get_hand_value(),
                          name=self.player_list[hand.player_identity].get_name(),
                          num=hand.player_identity+1))
        elif hand.standing:
            print(hand.hand)
            print('{name}, player #{num}, Standing with {hv}'
                  .format(hv=hand.get_hand_value(),
                          name=self.player_list[hand.player_identity].get_name(),
                          num=hand.player_identity+1))
    ...
    def make_player_move(self, hand):
        if self.input == 'hit':
            ...
            if self.player_list[hand.player_identity].balance() >= hand.bet and hand.can_split():
                self.split(hand)
            elif not hand.can_split():
                print('Cannot split a hand unless both cards are of the same value')
            else:
    ...
    def dealer_plays_hand(self):
        ...
        while self.dealer_hand.get_hand_value() < 16 or \
                (self.dealer_hand.is_hand_soft() and self.dealer_hand.get_hand_value() <= 16):
            print('Dealer has {dhv}, dealer hits'.format(dhv=self.dealer_hand.get_hand_value()))
            self.dealer_hand.hit(self.the_deck.next_card())
            print(self.dealer_hand.hand)
        if self.dealer_hand.get_hand_value() > 21:
            self.dealer_hand.bust = True
            print('Dealer has {dhv}, dealer busts!'.format(dhv=self.dealer_hand.get_hand_value()))
        else:
            print('Dealer has {dhv}, dealer stands'.format(dhv=self.dealer_hand.get_hand_value()))
